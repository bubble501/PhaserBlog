...
Game.Level1.prototype = {
    ...
    // lives text 
    livesText = this.game.add.text(0, this.game.height - 70, "Lives: ", {font: '32px Arial', fill:  '#fff'});
    // create player
    ...
    fireEnemyBullet: function() {
        ...
        if(this.time.now > enemyBulletTime) { 
            ...
            if(enemyBullet && livingEnemies.length > 0) {
                ...
                enemyBulletTime = this.time.now + 1000;
    ...
    collisionEnemyBullet: function(player, enemyBullets) {
        // minus lives by 1
        lives--;
        emitter.on = false;
        explosionSound.play();
        // set the flags canEnemyFire and canPlayerFire to false
        canEnemyFire = false;
        canPlayerFire = false;
        // conditional check if lives are greater than 0
        if (lives > 0) {
            button = this.add.button(this.world.centerX - 50, this.world.centerY, "button", this.actionOnClick, this, 1, 0, 2);
        } else {
            canEnemyFire = false;
            canPlayerFire = false;
            var gameOver = this.game.add.text(this.world.centerX - 50, this.world.centerY, "Game Over", {font: '32px Arial', fill:  '#fff'});
        }
